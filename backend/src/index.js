...
app.get("http://localhost:3000/logout", (req, res) => {
  ...
  req.session.destroy((err) => {
    if (err) {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Successful logout" });
    }
  });
});
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        res
          .status(500)
          .send({ message: "Fatal error: Insert operation failed" });
      } else {
...
app.post("/moredetails", (req, res) => {
  ...
  const gmapapiURL = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
    fulladdr
  )}&key=${gmapapikey}`;
  ...
  fetch(gmapapiURL)
    ...
    .then((data) => {
      ...
      // Address Table
      db.query(
        "INSERT INTO address (Address, Suburb, Postcode, Latitude, Longitude, userid) VALUES (?, ?, ?, ?, ?, ?)",
        [Address, Suburb, Postcode, latitude, longitude, id], // Handle for lat and long values
        (err, result) => {
          if (err) {
            console.log(err);
  ...
  // Payment Table
  db.query(
    "INSERT INTO payment (PaymentAmount, PaymentType, CardNo, CardExpiry, userId) VALUES (?, ?, ?, ?, ?)",
    [50, "Membership subscription payment", cardNo, cardExp, id], // Need to change paymentamount based on Subscription type
    (err, result) => {
      if (err) {
        console.log(err);
        res
  ...
  // Membership Table
  db.query(
    "INSERT INTO membership (membershipType, userId) VALUES (?, ?)",
    [memType, id],
    (err, result) => {
      if (err) {
        console.log(err);
